...
namespace Hourly_Ref_ET_Calculator
{
    public class HourlyRefET
    {
        ...
        public double delta_vapor { get; set; }
        public double delta_angle { get; set; }
        public double phi { get; set; }
        public double beta { get; set; }
        public double omega_s { get; set; }
        public double omega1 { get; set; }
        public double omega2 { get; set; }
        public double Ra { get; set; }
        public double Rso { get; set; }
        public double TKhr { get; set; }
        public double es { get; set; }
        public double ea { get; set; }
        public double Rns { get; set; }
        public double P { get; set; }
        public double gamma { get; set; }
        public double u2 { get; set; }
        public double Cn { get; set; }
        public double Cd { get; set; }
        public double sin_phi { get; set; }
        public double W_precip_water { get; set; }
        public double Kb { get; set; }
        public double Kd { get; set; }
        public double Rso_adv { get; set; }
        public double Rs_Rso_adv { get; set; }
        public double fcd_adv { get; set; }
        ...
        public double t_mid_time { get; set; }
        ...
        {
            Sc = Calc_Sc(J_doy);
            omega = Calc_omega(Lm_longitude, Lz_longitude, Sc, t_std_time);
            dr = Calc_dr(J_doy);
            delta_vapor = Calc_delta_vapor(Ta_air_Temperature);
            delta_angle = Calc_delta_angle(J_doy);
            phi = Calc_phi(phi_degree);
            beta = Calc_beta(phi, delta_angle, omega);
            omega_s = Calc_omega_s(phi, delta_angle);
            var omega_1n2 = Calc_omega1_omega2(omega, omega_s);
            omega1 = omega_1n2.Item1;
            omega2 = omega_1n2.Item2;
            Ra = Calc_Ra(dr, omega2, omega1, phi, delta_angle);
            Rso = Calc_Rso(Ra, z_elevation);
            TKhr = Calc_TKhr(Ta_air_Temperature);
            es = Calc_es(Ta_air_Temperature);
            ea = Calc_ea(RH_humidity, es);
            Rns = Calc_Rns(Rs_measured_rad);
            P = Calc_P(z_elevation);
            gamma = Calc_gamma(P);
            ...
            {
                CropName = "ETr";
                ETr = (0.408 * delta_vapor * (Rn - G) + gamma * Cn / (Ta_air_Temperature + 273) * u2 * (es - ea)) / (delta_vapor + gamma * (1 + Cd * u2));
            }
            else
            {
                CropName = "ETo";
                ETo = (0.408 * delta_vapor * (Rn - G) + gamma * Cn / (Ta_air_Temperature + 273) * u2 * (es - ea)) / (delta_vapor + gamma * (1 + Cd * u2));
            }
